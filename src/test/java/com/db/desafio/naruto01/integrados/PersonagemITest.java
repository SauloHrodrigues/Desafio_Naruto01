...
public class PersonagemITest {
    ...
    @Test
    @DisplayName("Deve adicionar um novo jutsu a um personagem já cadastrado")
    @Sql(scripts = "/gerar_banco.sql", executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD)
    @Sql(scripts = "/limpar_banco.sql", executionPhase = Sql.ExecutionPhase.AFTER_TEST_METHOD)
    public void deveAdicionarUmNovoJutsu() {

        Personagem personagem = repository.findByNomeIgnoreCase("Rock Lee");
        Long id = personagem.getId();

        NovoJutsu dto = new NovoJutsu("novo jutsu", 50);

        ResponseEntity<Void> resposta =
                template.postForEntity("/personagens/{id}/jutsus", dto, Void.class, id);
