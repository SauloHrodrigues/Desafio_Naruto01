...
public class JogoController {
    ...
    private final JogoServiceI serviceI;

    @PostMapping
    public ResponseEntity<Void> iniciarJogo(@RequestBody @Valid NovoJogoRequest dto) {
        serviceI.iniciarJogo(dto);
        return ResponseEntity.status(HttpStatus.NO_CONTENT).build();
    }

    @PostMapping("/{id}/ataque")
    public ResponseEntity<Void> usarJutsu(@PathVariable Long id, @RequestBody @Valid AtacarDeJutsuRequest dto) {
